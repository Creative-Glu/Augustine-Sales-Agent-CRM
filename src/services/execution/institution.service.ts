...
export async function getInstitutionPaginated({
  ...
  try {
    ...
    throw error instanceof Error ? error : new Error('getInstitutionPaginated failed');
  }
}
...
export async function getInstitutionById(id: number | string): Promise<Institution | null> {
  try {
    ...
    throw error instanceof Error ? error : new Error('getInstitutionById failed');
  }
}
...
export async function getInstitutionCounts(last24h: boolean): Promise<InstitutionCounts> {
  try {
    ...
    throw error instanceof Error ? error : new Error('getInstitutionCounts failed');
  }
}
