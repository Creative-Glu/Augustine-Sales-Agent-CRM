...
import { useJobsPaginated } from '@/services/execution/useExecutionData';
import { JobsFilters } from './ExecutionFilters';
...
export default function ExecutionJobsPage() {
  ...
  const basePath = pathname ?? '/execution-dashboard/jobs';
        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-between gap-4 flex-wrap py-4 border-b border-border/60">
            <p className="text-sm text-muted-foreground font-medium">
              Showing{' '}
              <span className="text-foreground">{jobsQuery.data?.data?.length ?? 0}</span>
              {' of '}
              <span className="text-foreground">{total}</span> jobs
            </p>
